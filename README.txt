...
$ python find_ligands_wrapper.py -- <PDB_ID> <CHAIN> <--save_dir> <--mw_filter> <--sequence_similarity>
    INPUTS:
      ...
      SEQ_SIM   == Sequence similarity allowed for searching for similar PDB_ID (i.e. 0.3 for 30% SS), default set to 0.9 or 90%
    OUTPUTS:
...
How it works
    $ find_ligands_wrapper.py
        1.) Find Similar PDBs (pdb_get_similars) using the MMseqs2 to search the PDB, by the fasta sequence of PDB_ID/CHAIN
